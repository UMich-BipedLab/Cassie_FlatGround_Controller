...
GL = load(fullfile('Controllers', 'Yukai', 'mat', 'GaitLibrary_WithTorso_OneStep_NoMatch_v3.mat'));

GL = GL.GaitLibrary;
for i = 1:length(GL)
    GL{i}.solution.states{1} = MirrorGaitStates(GL{i}.solution.states{3});
    GL{i}.solution.params{1}.atime = MirrorGaitParams(GL{i}.solution.params{3}.atime);
...
for i = 1:length(GL)
    GaitLibrary_joint.RightStance.HAlpha(i,:,:) =  reshape(GL{i}.opt.params{1}.atime,10,6);
    GaitLibrary_joint.LeftStance.HAlpha(i,:,:) =  reshape(GL{i}.opt.params{3}.atime,10,6);
    GaitLibrary_joint.Desired_Velocity(:,i) = GL{i}.velocity;
    GaitLibrary_joint.RightStance.ct(i) = 1/GL{i}.opt.params{1}.ptime(1);
    GaitLibrary_joint.LeftStance.ct(i) = 1/GL{i}.opt.params{3}.ptime(1);
end
...
for i = 1:length(GL)
    %Right Stance
    yaw = GL{i}.opt.states{1}.x(4,:);
    pitch = GL{i}.opt.states{1}.x(5,:);
    roll = GL{i}.opt.states{1}.x(6,:);
    dyaw = GL{i}.opt.states{1}.dx(4,:);
    dpitch = GL{i}.opt.states{1}.dx(5,:);
    droll = GL{i}.opt.states{1}.dx(6,:);
    ...
    roll = GL{i}.opt.states{3}.x(6,:);
    dyaw = GL{i}.opt.states{3}.dx(4,:);
    dpitch = GL{i}.opt.states{3}.dx(5,:);
    droll = GL{i}.opt.states{3}.dx(6,:);
...
save(fullfile('Controllers', 'Yukai', 'mat', 'GaitLibrary_output', 'GaitLibrary_output_up2'), 'GaitLibrary_output');
